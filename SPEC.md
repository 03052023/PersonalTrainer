...
| RF-40 | **Como fazer** (pedido do usuário, 2026-09-24): botão no exercício (sessão, Home, catálogo) que mostra uma ilustração **própria, minimalista e sem realismo** (manequim neutro de traço simples, sem músculos nem rosto, equipamento só sugerido), desenhada pelo app a partir de dados de pose e animada em loop entre a posição inicial e a final. Com Reduzir Movimento, mostra as duas posições lado a lado. Traz também 3 passos curtos e 2 erros comuns em pt-BR. Tudo offline, no mesmo formato para todos os exercícios, sem mídia de terceiros. Detalhes após a pesquisa em andamento. | v2.1 |
