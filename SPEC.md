...
| RF-42 | **Modo casa** (pedido do usuário, 2026-09-24): chave "Treinar em casa" em Ajustes e na tela Hoje.<br>Ligada, cada exercício da sessão planejada é trocado pelo equivalente que dá para fazer em casa sem equipamento específico: peso do corpo ou objetos comuns, como mochila com peso, garrafas de água, cadeira firme, degrau ou toalha (novo equipamento `household`).<br>A escolha é determinística (§7.13 H1–H4): o mesmo padrão de movimento e um grupo primário em comum, ordenados como no RF-34. Sem equivalente, vale o mesmo grupo primário; se nem isso existir, o exercício sai da sessão, com aviso.<br>O programa não muda: desligar a chave volta os exercícios da academia. Cada exercício tem o próprio histórico (P3), então quem nunca fez a versão de casa começa em calibração. O alvo (séries, faixa, RIR, descanso) vem do exercício original. O cartão do dia mostra "Em casa". Na sessão, o botão Trocar oferece só alternativas de casa. | v2.1 |
| RF-43 | **Medida do exercício** (achado de 2026-09-24): cada exercício do catálogo tem uma medida, `reps` (padrão), `seconds` (isometrias, pranchas) ou `steps` (carregadas). Prescrição, registro da série, histórico e resumos mostram a unidade certa ("3 × 20–40 s", "30 passos"). A progressão (P4–P6) usa o mesmo número com a mesma lógica. A medida e a marca "dá para fazer em casa" vêm do catálogo do seed pelo `slug`, sem mudar o esquema de dados; exercício personalizado usa `reps` e não entra no modo casa. | v2.1 |
| RF-41 | **RIR explicado onde é pedido** (pedido do usuário, 2026-09-24): (a) o seletor de RIR da série mostra o significado de cada valor ("0 · nenhuma a mais", "1 · mais uma", "2 · mais duas", "3+ · com folga"); (b) um botão "O que é RIR?" ao lado dele abre uma explicação curta com exemplo, a escala, a equivalência com RPE, a dica "na dúvida, chute para baixo" e as referências (`topic.rir`); (c) na primeira sessão, um cartão explica o RIR uma única vez antes da primeira série ("Entendi"); (d) na prescrição, "RIR 2" tem a leitura acessível "parar com 2 repetições de reserva". | v2.1 |
...
| Regra | Descrição |
|-------|-----------|
| **H1 Quem é de casa** | Um exercício é "de casa" quando o catálogo do seed o marca `atHome: true`. Isso vale para peso do corpo sem aparelho (flexão, agachamento, prancha, ponte) e para objetos comuns (`household`: mochila com peso, garrafas, cadeira, degrau, toalha). Peso do corpo que exige aparelho (barra fixa, paralelas, banco 45°, caixa de salto) não é de casa. |
| **H2 Equivalente** | Para cada exercício do plano: se ele já é de casa, fica. Senão, o equivalente é o melhor candidato de casa com o mesmo padrão de movimento e ao menos um grupo primário em comum, na ordem do RF-34 (pontuação, afinidade de equipamento, nome, id). Sem candidato: o melhor de casa com o mesmo grupo primário. Sem nenhum: o exercício sai da sessão com o aviso "sem opção em casa para X". |
...
| **H4 Alvo e histórico** | O alvo (séries, faixa, RIR, descanso) vem do exercício original. A carga e a nota vêm do histórico do exercício de casa (P1–P12); sem histórico, `calibrate`. Peso do corpo com carga 0 segue P8 (a progressão sobe repetições e, no topo da faixa, sugere carga adicional, por exemplo uma mochila). |
