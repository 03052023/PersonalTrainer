...
- **Ícone:** o usuário escolheu por último o **conceito A** (5 pétalas separadas em forma de gota, cada uma de uma cor, miolo claro) **sobre azul profundo** (`#3E5F7E` → `#2C4763`), com pétalas **discretas, próximas do fundo** (azuis acinzentados, marrons e terrosos queimados). Candidatos em `docs/design/candidates/icon-muted-m1..m4.png` e `muted-compare.png`; gerador em `docs/design/render-a-muted.ps1` (PowerShell + System.Drawing, 1024 px). **Pendente: ele escolher entre m1, m2, m3 e m4** (ou pedir ajuste). Ordem das pétalas (horária a partir do topo): Longevidade, Hipertrofia, Força, Combate, Resistência; cada pétala vira a cor daquele objetivo no app.
- Histórico descartado: halter azul (M1), marrom-bege, rosácea creme (conceito B) em musgo e azuis, conceito A em azuis vivos.
